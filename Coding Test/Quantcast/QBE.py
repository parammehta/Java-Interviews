...
def insert(query, JSONstore, timeStamp):
    ...
    if query.__contains__('location'):
        ...
        if query.__contains__('location'):
            query = query.strip('insert ').strip('}').strip(' {').replace('"', '').replace('location:{', '').replace(
                '}', '').split(',')
        else:
...
def get(query, JSONstore, timeStamp):
    ...
    if query.__contains__('type'):
        ...
        idx = []
        if query.__contains__('location'):
            search_params = query.strip('get ').strip('}').strip(' {').replace('"', '').replace('location:{',
                                                                                                '').replace('}',
                                                                                                            '').split(
                ',')
        else:
...
def delete(query, JSONstore, timeStamp):
    ...
    idx = []
    if query.__contains__('location'):
        search_params = query.strip('delete ').strip('}').strip(' {').replace('"', '').replace('location:{',
                                                                                               '').replace('}',
                                                                                                           '').split(
            ',')
    else:
